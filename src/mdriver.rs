...
use mochios_mdriver_protocol::control::{
    ...
    MDRIVER_CONTROL_REGISTER_DISPLAY_BUFFER, MDRIVER_CONTROL_START_SESSION,
    MDRIVER_CONTROL_STATUS_END, MDRIVER_CONTROL_STATUS_OK, MDRIVER_CONTROL_VERSION,
    ...
    MDRIVER_DISPLAY_BULK_PAGE_COUNT, MDRIVER_DISPLAY_GPU_SCENE_COMMIT_WIDTH,
    MDRIVER_DISPLAY_GPU_SCENE_COMMIT_X, MDRIVER_DISPLAY_MAX_TRANSFER,
...
use mochios_mdriver_protocol::{
    BLOCK_DATA_PAGE as MDRIVER_BLOCK_DATA_PAGE, CONTROL_IRQ_PORT as MDRIVER_CONTROL_IRQ_PORT,
    CONTROL_PORT as MDRIVER_CONTROL_PORT, CONTROL_REQUEST_KIND as MDRIVER_CONTROL_REQUEST_KIND,
    CONTROL_RESPONSE_KIND as MDRIVER_CONTROL_RESPONSE_KIND,
    CONTROL_RING_GENERATION as MDRIVER_CONTROL_RING_GENERATION,
    CONTROL_RING_PORT as MDRIVER_CONTROL_RING_PORT, DOMAIN_ID as MDRIVER_DOMAIN_ID,
    INSTALL_METADATA_PAGE as MDRIVER_INSTALL_METADATA_PAGE,
};
...
pub fn storage_control(
    ...
    use mnu_abi::{
        ...
        STORAGE_CONTROL_INSPECT, STORAGE_CONTROL_INSTALL_PARTITION,
    };
    ...
    if !matches!(
        operation,
        STORAGE_CONTROL_INSPECT
            | STORAGE_CONTROL_CREATE_PARTITION
            | STORAGE_CONTROL_DELETE_PARTITION
            | STORAGE_CONTROL_INSTALL_PARTITION
    ) {
        return Err(Error::Protocol);
    }
    ...
    if operation == STORAGE_CONTROL_INSTALL_PARTITION {
        if arguments[3] != 0 {
            return Err(Error::Protocol);
        }
        arguments[3] = u64::from(install_metadata_reference(client)?);
    }
    let mdriver_operation = match operation {
        STORAGE_CONTROL_INSPECT => MDRIVER_CONTROL_INSPECT_STORAGE,
        STORAGE_CONTROL_CREATE_PARTITION => MDRIVER_CONTROL_CREATE_PARTITION,
        STORAGE_CONTROL_DELETE_PARTITION => MDRIVER_CONTROL_DELETE_PARTITION,
        STORAGE_CONTROL_INSTALL_PARTITION => MDRIVER_CONTROL_INSTALL_PARTITION,
        _ => return Err(Error::Protocol),
    };
    transact(
        client,
        MdriverControlRequest::new(mdriver_operation, device_id, arguments),
    )
}
...
fn install_metadata_reference(client: &mut Client) -> Result<u32, Error> {
    ...
    let digest = mnu::init::fs::kernel_read_initfs("/install/rootfs.sha256")
...
fn ensure_display(client: &mut Client) -> Result<DisplayState, Error> {
    ...
    let (grant_start, grant_size) =
        hypervisor_guest::grant_window().ok_or(Error::MissingGrantWindow)?;
    let bulk_pages_end = MDRIVER_DISPLAY_BULK_FIRST_PAGE + MDRIVER_DISPLAY_BULK_PAGE_COUNT as u64;
...
fn platform_error(_error: Error) -> mnu::platform::PlatformError {
    mnu::platform::PlatformError::Transport
}

fn platform_display_info() -> Result<mnu::platform::DisplayInfo, mnu::platform::PlatformError> {
    display_info()
        .map(|info| mnu::platform::DisplayInfo {
            width: info.width,
            height: info.height,
            stride: info.stride,
